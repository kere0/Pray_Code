...
public class HunterRightHand : MonoBehaviour
{
    private HunterController hunter;
    private float force = 0.1f;
    public Collider hunterHandCollider;
    private void Awake()
    {
        TryGetComponent(out hunterHandCollider);
        hunter = transform.root.GetComponent<HunterController>();
    }
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (player != null)
            {
                hunterHandCollider.enabled = false;
                CombatEvent combatEvent = new CombatEvent();
                combatEvent.Sender = hunter;
