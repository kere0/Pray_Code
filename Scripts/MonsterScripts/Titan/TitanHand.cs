...
{
    private TitanController titan;
    private float force = 0.2f;
    public Collider titanHandCollider;
    private void Awake()
    {
        TryGetComponent(out titanHandCollider);
        titan = transform.root.GetComponent<TitanController>();
    }
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (player != null)
            {
                titanHandCollider.enabled = false;
                CombatEvent combatEvent = new CombatEvent();
                combatEvent.Sender = titan;
