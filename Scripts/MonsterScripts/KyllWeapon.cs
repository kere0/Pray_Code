...
public class KyllWeapon : MonoBehaviour
{
    private KryllController kryll;
    private float force = 0.1f;
    public Collider KyllWeaponCol;
    private void Awake()
    {
        TryGetComponent(out KyllWeaponCol);
        kryll = transform.root.GetComponent<KryllController>();
    }
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (player != null)
            {
                KyllWeaponCol.enabled = false;
                CombatEvent combatEvent = new CombatEvent();
                combatEvent.Sender = kryll;
