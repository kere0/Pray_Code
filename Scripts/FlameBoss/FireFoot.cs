...
public class FireFoot : MonoBehaviour
{
    public Collider fireFootCollider;
    private float force = 0.3f;
    void Awake()
    {
        TryGetComponent(out fireFootCollider);
    }

    private void OnEnable()
    {
        fireFootCollider.enabled = true;
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (player != null)
            {
                fireFootCollider.enabled = false;
