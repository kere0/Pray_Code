    ...
    void Update()
    {
        while (combatEventQueue.Count > 0)
        {
            CombatEvent combatEvent = combatEventQueue.Dequeue();
                ...
                combatEvent.Receiver.TakeKnockback(combatEvent.KnockbackForce, combatEvent.KnockbackDuration);
            
            
            combatEvent.Receiver.TakeDamage(combatEvent.Damage, true, combatEvent.isGuardIgnore);
        }
    }
